...
## 測試 Timeout 規範（強制）
- ⚠️ **任何涉及背景服務（IHostedService）、WebSocket、多執行緒（Task.Run/Task.WhenAll）或非同步等待的測試，必須在撰寫時就加上雙重 timeout 保護**：
  1. `[Fact(Timeout = 15000)]`（xUnit 層級 15 秒硬性上限）
  2. `using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));` 並將 `cts.Token` 傳入所有 await
- 如果測試啟動了 IHostedService 的背景迴圈（StartAsync），**必須在測試結束前呼叫 StopAsync 並等待完成**
- WebSocket 的 CloseAsync 會等待 server 回應，改用 `CloseOutputAsync`（只送不等）
- 這是此專案重複出現多次的 CI 卡住根因，不要等 CI 卡住才補

## 指令執行規範
- ⚠️ **嚴禁使用分號（`;`）串接任何指令**，違反此規則會觸發 PowerShell 安全警告，每次都需要手動確認，影響自動化流程。
- ⚠️ **嚴禁使用 `&&` 串接多個指令**（唯一例外：`git add && git commit` 這種不可分割的原子操作）。
- 每個指令必須獨立執行，等上一個完成再執行下一個。
- `cd` 和後續指令必須分開呼叫，不得合併在同一行。
