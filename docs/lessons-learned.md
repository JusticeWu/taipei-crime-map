...
## L023：光暈效果參數選在範圍上限，密集同色點位下視覺上類似熱力圖殘留

- 問題：使用者回報「點位圖模式下出現了熱力圖的綠色半透明圓圈疊加在一起」，
  並要求檢查 map.js 的 heat/point 圖層切換邏輯（removeLayer/clearLayers）
- 根本原因：逐一檢視 `clearLayers()`、`update()`、`startProgressiveLoad()`、
  `setHeatmap()`、`queryHeatmapOnly()`、`queryProgressive()`、`onModeChange()`，
  確認每個模式切換入口都會呼叫 `clearLayers()` 移除 `_heatLayer`，
  且非同步回應皆有 `generation` 計數器守門，並未發現圖層未移除的程式碼缺陷。
  真正原因是 PR #36 新增的 `filter: drop-shadow` 光暈，當時把 blur/opacity
  設在規格範圍的上限（5px / 0.4），在大量同色點位（住宅竊盜為多數案類，
  顏色 #1E8449 綠色）密集重疊時，光暈彼此疊加，視覺上會形成類似熱力圖
  半透明色塊的效果，使用者誤判為「熱力圖層未被移除」
- 正確做法：
  - 收到「圖層未正確切換/殘留」的回報時，先以程式碼逐一核對所有
    `removeLayer`/`clearLayers` 呼叫點與非同步守門機制，確認邏輯本身無誤後，
    再考慮是否為「視覺效果疊加」造成的誤判
  - 光暈類效果的模糊半徑/透明度，應以「大量同色點位密集重疊」的情境
...
## L025：上一輪「群聚圖示白色外框」根因判斷錯誤，實際是 .district-bubble 的白色 border

- 問題：L024 推測根因為 Leaflet `DivIcon` 預設 `.leaflet-div-icon`
  （`background:#fff; border:1px solid #666;`）套用到群聚圖示，並在
  style.css 加上 `!important` 覆蓋；但使用者實際用 Chrome DevTools
  Inspect 後找到的真正根因，是 map.js 內 `.district-bubble` 規則裡
  `border: 2px solid rgba(255,255,255,.75)`（PR #36 實作光暈效果時誤加）
- 根本原因：`.district-bubble` 是「無精確座標案件」的行政區聚合圓形圖示，
  與 MarkerCluster 的群聚圓圈外觀類似（皆為深色圓形數字標記），
  容易被誤認為同一種元件；前一輪未實際在瀏覽器 Inspect，
  僅憑程式碼與第三方 CSS 比對，誤判為 Leaflet 預設樣式造成
- 正確做法：
  - 當畫面上有「外觀相似但屬於不同元件」的圖示（群聚圓圈 vs 行政區
    聚合圓圈）時，回報問題前應先確認使用者點到的究竟是哪一個元件
  - 沒有瀏覽器可用時的 CSS 比對僅能作為「假設」，使用者用 DevTools
    Inspect 找到的「Computed/Styles 面板實際命中的規則」才是最終依據，
    應以使用者的 Inspect 結果為準並直接修正
  - 移除已確認非根因的修正前，需先評估是否為無害的防禦性程式碼；
    若僅是基於錯誤假設、且不影響功能，可保留但應在文件中註記實際根因
    已改變，避免誤導未來維護者
- 相關模式：[[L024]]（修正方向錯誤，但工具受限造成）；
  外觀相似的不同元件（群聚圖示 vs 行政區聚合圖示）容易混淆，
  修正前應先確認是「同一個元件」
