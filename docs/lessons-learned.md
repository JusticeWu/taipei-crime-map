...
## L042：手機版「內部捲動容器」會攔截整頁捲動手勢，應優先讓頁面自然變長
- 問題：手機版統計圖表從 2 個增加到 5 個後，為避免撐爆畫面，設定了 `max-height` + `overflow-y:auto`（內部捲動容器）。這解決了「畫面被撐爆」的問題，但同時製造了新問題：使用者的滑動手勢被地圖（Leaflet 觸控平移）和統計區塊（內部捲動）各自攔截，導致無法捲動整個頁面，使用者卡在兩個區塊之間動彈不得。原本打算用「新增浮動跳轉按鈕」這個更複雜的方案來補救，但後來發現只要移除「內部捲動」設定本身，讓頁面自然變長、改回靠整頁捲動瀏覽，問題就直接消失了，完全不需要額外的按鈕功能。
- 根本原因：「內容變多導致畫面被撐爆」不一定要靠「限制容器高度 + 內部捲動」解決。手機版本來就習慣靠整頁垂直捲動瀏覽，讓頁面自然變長、交給瀏覽器原生的整頁捲動處理，比自己刻意做一層內部捲動容器更簡單、更符合使用者習慣。內部捲動容器在手機版有一個容易被忽略的副作用：它會跟手勢操作型的元件（例如地圖的觸控平移）互相打架，攔截使用者原本想用來捲動整頁的滑動手勢。
- 正確做法：手機版讓頁面自然變長，移除 `max-height` 和 `overflow-y:auto`，改為 `height: auto; overflow: visible`。桌面版的側邊欄因為有固定空間配置，維持 `overflow-y: auto` 內部捲動是合理的——兩者的使用情境不同，不要共用同一條規則。
- 相關模式：[[L023]] flex 在手機版的行為差異；發現一個解法製造了新問題時，先重新檢視「問題的根源是不是在更早一步的設計選擇上」，而不是急著在新問題上再疊加一個更複雜的解法（例如本次想用浮動按鈕去補救，但只要撤掉造成問題的設定就好）。

## L043：延長快取 TTL 會意外打斷依賴「API 一定會觸發 DB 連線」的 Keepalive 機制
- 問題：將 `/api/crime/stats` 的快取從 L1(1分鐘)/L2(30分鐘) 改為 L1(15小時)/L2(24小時) 後，Cron-job.org 每 15 分鐘呼叫的 keepalive 幾乎每次都命中快取，不會真正建立資料庫連線。Azure SQL Database 免費層因為長時間沒有實際連線而自動暫停，keepalive 機制實質上失效。
- 根本原因：調整快取 TTL 時，只考慮了「減少 DB 負載」這個目標，沒有檢查是否有其他機制依賴該 API「每次呼叫一定會觸發底層資源（DB/外部服務）連線」這個隱含行為。快取的存在改變了 API 呼叫的副作用——原本每次都連 DB 的行為變成了大多數時候直接回快取，但 keepalive 依賴的恰恰是「連 DB」這個副作用。
- 正確做法：另開一個不受快取影響的專用端點（`/api/health/db-ping`，每次都執行 `SELECT 1`），keepalive 改打這個端點。不要為了遷就 keepalive 而縮短快取 TTL，那樣會犧牲快取優化的成果。同時加上 X-API-Key 驗證和 Rate Limiting，防止被外部濫用。
- 相關模式：任何改變 API 內部行為（加快取、加佇列、改為非同步）的優化，都要檢查是否有外部系統依賴原本的「同步、每次都執行」行為，包括監控探針、keepalive、webhook 確認等。
