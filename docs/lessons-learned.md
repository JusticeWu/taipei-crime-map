...
## L026：大量小修正疊加 5 小時仍無法解決白色方塊與熱力圖/點位圖混層問題，決定整批 revert 重做

- 問題：UAT 上「點位旁白色小方塊（非光暈）」與「深灰熱力圖群聚圓圈混入點位圖」
  兩個問題，從 PR #29 之後歷經 PR #30~#37 共 8 個 PR、20 個 commit、
  超過 5 小時的逐項修正（顏色邏輯、popup 欄位、分布圖、光暈效果、
  cache 版本、群聚圖示外框等）仍未解決，地圖視覺異常不適合 merge 到 Prod
- 根本原因：問題範圍橫跨多個 PR 疊加的程式碼（光暈 CSS、群聚 iconCreateFunction、
  熱力圖/點位圖切換邏輯），逐項小修正每次只解決「症狀」的一部分，
  未能定位到真正的結構性問題；持續疊加修正反而讓程式碼與假設越來越複雜，
  難以判斷哪個改動才是根因
- 正確做法：
  - 對 `a132df5..HEAD`（PR #29 之後共 21 個 commit）執行
    `git revert --no-commit a132df5..HEAD`，未發生衝突，
    revert 後的 tree 與 `a132df5` 完全一致（`git diff --cached a132df5` 為空）
  - revert 前先打上備份 tag `pre-revert-pr29-backup` 並 push 到遠端，
    確保有 PR #30~#37 的完整內容可隨時找回
  - 重做時拆成 4 個獨立小項目（群聚顏色邏輯、點位顏色、popup 欄位、
    行政區/時段分布圖），每完成一項就測試確認再做下一項，
    避免重蹈「多項修正疊加、難以追溯根因」的覆轍
- 額外發現：revert 後 CI 的「Find latest report」步驟因為
  `git diff HEAD~1 HEAD --name-only` 抓到「被刪除」的報告檔案，
  `cat` 不存在的檔案導致整個 deploy-to-uat job 失敗
  （但實際部署本身已成功）。修正為 `git diff --diff-filter=ACMR`，
  排除已刪除的檔案
- 相關模式：[[L023]][[L024]][[L025]]；當同一個視覺問題被多輪小修正
  仍無法解決時，應考慮「整批 revert 到已知良好版本，重新以更小的步驟
